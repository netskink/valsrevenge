        ...
        for col in 0..<numberOfColumns {
            for row in 0..<numberOfRows {
                if let td = tileDefinition(atColumn: col, row: row) {
                    if let bodySizeValue = td.userData?.value(forKey: "bodySize") as? String { // gets the key from user data
                        let x = CGFloat(col) * tileSize.width - halfWidth + (tileSize.width/2)
                        let y = CGFloat(row) * tileSize.height - halfHeight + (tileSize.height/2)
                        
                        let bodySize = NSCoder.cgPoint(for: bodySizeValue)  // reads the value in user data
